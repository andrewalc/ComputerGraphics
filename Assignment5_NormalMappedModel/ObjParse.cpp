...
void ObjParse::parse(std::string fileName) {
        ...
        while (getline(inFile, line)) {
            ...
            if (tag == "vn") {
                ...
                while (i < line.length()) {
                    if (line.at(i) == '/' || line.at(i) == ' ' ||
                        line.at(i) == '\n') {
                        vertNormals.push_back(std::stof(numBuffer));
                        numBuffer = "";
                    } else {
                        // add the char to the buffer
                        numBuffer += line.at(i);
    ...
    QVector<QVector3D> normals = getNormals3D();
    for (int i = 0; i < faces.size(); i++) {         // each line
        for (int j = 0; j < faces[i].size(); j++) {  // each num
            if (j % 3 == 0) {
                ...
                QVector3D normal = normals.at(faces[i][j + 2] -1);
                Vertexture vt =
...
QVector<QVector3D> ObjParse::getNormals3D() {
    QVector<QVector3D> normVector;
    QVector3D vector;
