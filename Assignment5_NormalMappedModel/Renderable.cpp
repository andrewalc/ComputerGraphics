...
void Renderable::init(const QVector<Vertexture>& vts,
    ...
    for (int i = 0; i < numVerts; ++i) {
        ...
        data[i * vertexSize_ + 3] = vts.at(i).nx;
        data[i * vertexSize_ + 4] = vts.at(i).ny;
        data[i * vertexSize_ + 5] = vts.at(i).nz;
        data[i * vertexSize_ + 6] = vts.at(i).a;
        data[i * vertexSize_ + 7] = vts.at(i).b;
    }
    ...
    shader_.setAttributeBuffer(1, GL_FLOAT, 3 * sizeof(float), 3,
                               vertexSize_ * sizeof(float));
    shader_.enableAttributeArray(2);
    shader_.setAttributeBuffer(2, GL_FLOAT, (3 + 3) * sizeof(float), 2,
                               vertexSize_ * sizeof(float));
...
void Renderable::draw(const QMatrix4x4& world, const QMatrix4x4& view,
