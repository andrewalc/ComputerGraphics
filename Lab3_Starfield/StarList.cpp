...
StarList::StarList(unsigned int numStars, float spread, float speed)
    : spread_(spread), speed_(speed) {
...
void StarList::updateAndRender(QImage& image, float delta,
    ...
    // TODO: Modify me
    float tanHalfFOV = tan((70 / 2 * M_PI) / 180.0f);
    ...
    for (int i = 0; i < stars_.size(); i++) {
        ...
        int x =
            (int)((stars_[i].x / (givePerspective)) * halfWidth + halfWidth);
        int y =
            (int)((stars_[i].y / (givePerspective)) * halfHeight + halfHeight);
